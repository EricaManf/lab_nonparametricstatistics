...
This can be manually implemented in a straightforward way. Spoiler alert: the code below is very bad in terms of efficiency, it is only shown for didactic purposes!
...
Last note: a very nice interactive visualization on how local regression works is available [here](https://bookdown.org/egarpor/PM-UC3M/npreg-kre.html#npreg-kre-locpoly) (Figure 6.6). 
...
computationally more efficient B-spline basis (more on this later in the lab).
...
Example: if we want $4$ knots and a cubic
spline, we will end up with $7$ degrees of freedom.
...
natural spline basis, placing knots at all the inputs. This means that, remarkably, the problem defined on an infinite-dimensional function space has a finite-dimensional, unique minimizer which is a natural cubic spline! The this result is not even difficult to prove (see Theorem 2.3 of Nonparametric Regression and Generalized Linear Models: A Roughness Penalty Approach by Green and Silverman)
This is automatically performed in R through the `smooth.spline`
function.
...
problem (see e.g., Section 5.4.1 of The Elements of Statistical Learning by Hastie, Tibshirani and Friedman for further explanation).
