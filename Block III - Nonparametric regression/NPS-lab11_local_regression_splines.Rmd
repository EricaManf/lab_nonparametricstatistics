...
Local regression involves computing the fit at a target point $x_0$ using only the nearby training observations. Local regression is sometimes referred to as a *memory-based* procedure because, like nearest-neighbors, we
need all the training data each time we wish to compute a prediction. Let us keep working with the Prestige dataset. We start with weighted local averaging with uniform (or rectangular) kernel.
...
The reason is due to the different set of basis used to represent the space of spline functions: the former employs a conceptually simple (and easy to code) truncated power basis, whereas the latter uses a computationally more efficient B-spline basis.
...
I can either specify the knots (as done so far), or have R automatically select the spacing, I just need to specify their number knowing that:
\[\#dof = \#breaks + degree\]
Example, if we want $4$ breaks and a cubic spline, we will end up with $7$ degrees of freedom.
...
Despite the considerable coding effort with respect to using the built-in `bs()` function, values in the coefficients vector and in the design matrix get absurdly low and high in our hard-coded solution... Let us stick with B-spline bases from now on!
...
With smoothing splines we look for a function $f(\cdot)$ that makes $RSS=\sum_{i=1}^n(y_i-f(x_i))^2$ small,
but that is also smooth. That is, we aim at solving

\begin{equation}
\operatorname{RSS}(f, \lambda)=\sum_{i=1}^{n}\left\{y_{i}-f\left(x_{i}\right)\right\}^{2}+\lambda \int\left\{f^{\prime \prime}(t)\right\}^{2} d t
\end{equation}

This is operationally done by performing a penalized regression over the natural spline basis (orthonormal), placing knots at all the inputs. This is automatically performed in R through the `smooth.spline` function.
...
where $\boldsymbol{A}$ is the hat matrix of the associated regression problem. 
```{r}
