...
In "easy" cases the power can be assessed analytically, for Wilcoxon and Permutation tests, we need t
...
for(ii in 1:length(delta_grid)){
  ...
  for(j in 1:B){
    ...
    x.1 <- rstable(n,1.5,0)
    x.2 = rstable(n,1.5,0,delta=delta)
...
```
Let's add some spice for this last, LONG calculation...
...
```
So, apart from wasting some time, you're not actually losing much... ;)
